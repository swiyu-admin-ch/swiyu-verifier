...
- **Breaking — Docker image:** the published image is now hardened. The default
  ...
  remains available under the `-legacy` suffix
  (`ghcr.io/swiyu-admin-ch/swiyu-verifier:<tag>-legacy`). Operators who cannot
  immediately adopt the hardened runtime **must pin to the `-legacy` tag** until
  they have completed the migration steps in
  [`migration-guides/v2.x-to-v3.0.0.md`](migration-guides/v2.x-to-v3.0.0.md);
  the `-legacy` variant will be removed in a later release. `(#834)`.
