...
| SWIYU_TRUST_REGISTRY_API_URL       | (Only set for testing purpose) Base URL of the TMS B2B API (IF-014) used to fetch idTS and piaTS Statements.                                                                                                                                                                            | URL               | none         |
| SWIYU_TMS_AUTHORING_URL            | (Only set for testing purpose) Base URL of the TMS B2B Authoring API (IF-014) used for On-the-Fly vqPS registration. If not set, the vqPS registration feature is disabled.                                                                                                             | URL               | none         |
| SWIYU_TMS_OAUTH_TOKEN_URL          | (Only set for testing purpose) OAuth2 token endpoint used to obtain an access token for the TMS B2B Authoring API. Required when `SWIYU_TMS_AUTHORING_URL` is set.                                                                                                                      | URL               | none         |
| SWIYU_TMS_BOOTSTRAP_REFRESH_TOKEN  | (Only set for testing purpose, secret) Initial OAuth2 refresh token for the TMS API. Rotated tokens are persisted in the `token_set` table and shared across pods.                                                                                                                      | string            | none         |
