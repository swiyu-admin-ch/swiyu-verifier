...
| ADDITIONAL_AUDIT_INFORMATION_VP_TOKEN_ENABLED | Includes the raw `vp_token` (full presentation as sent by the wallet) in `wallet_response` of the management API response. Disabled by default to keep the response minimal (non-breaking, EIDOMNI-1321).                                                                                                                                                     | bool               | false             |
| ADDITIONAL_AUDIT_INFORMATION_CREDENTIAL_SUBJECT_DATA_ENABLED | Includes `credential_subject_data` (requested claims) in `wallet_response` of the management API response. Disabled by default to keep the response minimal (non-breaking, EIDOMNI-1321).                                                                                                                                                      | bool               | false             |
| ADDITIONAL_AUDIT_INFORMATION_CREDENTIAL_EVALUATION_ENABLED | Includes `credential_evaluation` (per-credential trust and status verification results) in the management API response. Disabled by default to keep the response minimal (non-breaking, EIDOMNI-1321).                                                                                                                                           | bool               | false             |
